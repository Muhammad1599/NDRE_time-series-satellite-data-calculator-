...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('ndre_analysis.log'),
...
def fill_gaps(df: pd.DataFrame, max_gap_days: int = 32) -> pd.DataFrame:
    """
    Fill temporal gaps in NDRE time series using a combination of
    Whittaker smoothing and spline interpolation.
    
    Args:
        df: Input DataFrame with date and NDRE columns
    ...
    gaps = merged_df['NDRE_mean'].isnull()
    ...
    # Apply Whittaker smoothing to mean NDRE
    x = merged_df['NDRE_mean'].fillna(merged_df['NDRE_mean'].mean()).values
    smoothed_ndre = whittaker_smooth(x, weights)
    
    # Use spline interpolation for final adjustments
    valid_idx = ~merged_df['NDRE_mean'].isnull()
    if valid_idx.sum() > 3:  # Need at least 4 points for cubic spline
        spline = CubicSpline(
            np.arange(len(merged_df))[valid_idx],
            merged_df['NDRE_mean'][valid_idx],
            bc_type='natural'
        )
        # Blend smoothed and spline results
        alpha = 0.7  # Weight for smoothed values
        merged_df.loc[merged_df['NDRE_mean'].isnull(), 'NDRE_mean'] = \
            alpha * smoothed_ndre[merged_df['NDRE_mean'].isnull()] + \
            (1 - alpha) * spline(np.arange(len(merged_df))[merged_df['NDRE_mean'].isnull()])
    else:
        # If too few points, use only Whittaker smoothing
        merged_df.loc[merged_df['NDRE_mean'].isnull(), 'NDRE_mean'] = \
            smoothed_ndre[merged_df['NDRE_mean'].isnull()]
    
    # Interpolate other columns
    for col in ['NDRE_min', 'NDRE_max', 'NDRE_stdDev']:
...
class NDREAnalyzer:
    """Class to handle NDRE analysis operations"""
    ...
    def calculate_ndre(self, image):
        ...
        try:
            ...
            return ndre.rename('NDRE')
        except Exception as e:
            logging.error(f"Error calculating NDRE: {e}")
            raise
    ...
    def process_date_chunk(self, geometry: ee.Geometry, start_date: str, end_date: str) -> List[Dict]:
        ...
        try:
            ...
            def process_image(image):
                # Calculate NDRE
                ndre = self.calculate_ndre(image)
                
                # Calculate statistics
                stats = ndre.reduceRegion(
                ...
                return ee.Feature(None, {
                    ...
                    'NDRE_stdDev': stats.get('NDRE_stdDev'),
                    'NDRE_min': stats.get('NDRE_min'),
                    'NDRE_max': stats.get('NDRE_max'),
            ...
            for feature in features:
                if feature['properties']['NDRE_mean'] is not None:
    ...
    def plot_time_series(self, df: pd.DataFrame, output_file: str):
        ...
        if 'is_interpolated' in df.columns:
            ...
            # Plot actual data
            ax1.plot(actual['date'], actual['NDRE_mean'], 'b-', 
                    label='Mean NDRE (Observed)', linewidth=2, alpha=0.7)
            ax1.scatter(actual['date'], actual['NDRE_mean'], 
                       color='blue', s=100, zorder=5, label='Observation')
            
            # Plot interpolated data
            if not interp.empty:
                ax1.plot(interp['date'], interp['NDRE_mean'], 'b--', 
                        label='Mean NDRE (Interpolated)', linewidth=1, alpha=0.5)
                ax1.scatter(interp['date'], interp['NDRE_mean'], 
                          color='lightblue', s=50, zorder=4, 
                          label='Interpolated', alpha=0.5)
        else:
            # Original plotting code for non-interpolated data
            ax1.plot(df['date'], df['NDRE_mean'], 'b-', 
                    label='Mean NDRE', linewidth=2, alpha=0.7)
            ax1.scatter(df['date'], df['NDRE_mean'], 
                       color='blue', s=100, zorder=5, label='Observation')
        
        # Add error bands
        ax1.fill_between(df['date'], 
                        df['NDRE_mean'] - df['NDRE_stdDev'],
                        df['NDRE_mean'] + df['NDRE_stdDev'],
                        alpha=0.2, color='blue', label='±1 StdDev')
        
        # Add min/max as points with connecting lines
        ax1.plot(df['date'], df['NDRE_min'], 'r--', alpha=0.5, linewidth=1, label='Min NDRE')
        ax1.plot(df['date'], df['NDRE_max'], 'g--', alpha=0.5, linewidth=1, label='Max NDRE')
        ax1.scatter(df['date'], df['NDRE_min'], color='red', s=50, alpha=0.5, zorder=4)
        ax1.scatter(df['date'], df['NDRE_max'], color='green', s=50, alpha=0.5, zorder=4)
        ...
        ax1.set_title('NDRE Time Series Analysis', pad=20, fontsize=14)
        ax1.set_ylabel('NDRE Value', fontsize=12)
        ax1.grid(True, alpha=0.3)
        ax1.legend(loc='center left', bbox_to_anchor=(1, 0.5))
        
        # Set y-axis limits with some padding
        ymin = df['NDRE_min'].min() - 0.1
        ymax = df['NDRE_max'].max() + 0.1
    ...
    def validate_results(self, df: pd.DataFrame) -> Tuple[bool, List[str]]:
        ...
        # Check for unrealistic NDRE values (typically between -1 and 1)
        if (df['NDRE_mean'] > 1).any() or (df['NDRE_mean'] < -1).any():
            issues.append("Unrealistic NDRE values detected")
    ...
    def run_analysis(self, geojson_path: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        try:
            ...
            csv_file = self.output_dir / f'ndre_results_{timestamp}.csv'
            plot_file = self.output_dir / f'ndre_timeseries_{timestamp}.png'
            ...
            metadata = {
                ...
                'mean_ndre': float(df_filled['NDRE_mean'].mean()) if not df_filled.empty else None,
